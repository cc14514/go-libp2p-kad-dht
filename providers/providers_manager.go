...
func (pm *ProviderManager) getProvidersForKey(k []byte) ([]peer.ID, error) {
	...
	if pset.ttl > 0 {
		...
		for _, p := range pset.providers {
			t, ok := pset.set[p]
			stat := "exclude"
			_period := time.Since(t)
			if ok && _period <= pset.ttl {
				ret = append(ret, p)
				stat = "setup"
			}
			log.Infof("providersForKey-filter-ttl : ttl=%v , pttl=%v  , stat=%s , peer=%s", pset.ttl, _period, stat, p.Pretty())
		}
	} else {
		ret = pset.providers
	}
...
func loadProviderSet(dstore ds.Datastore, k []byte) (*providerSet, error) {
	// add by liangc >>>>>>>>>
	tt := time.Duration(0)
	if hk, err := multihash.Cast(k); err == nil {
		nsv := os.Getenv(hk.String())
		if nsv != "" {
			if _tt, ok := new(big.Int).SetString(nsv, 10); ok {
				tt = time.Duration(_tt.Int64())
			}
		}
		log.Debugf("loadProvSet-set-ttl : cid=%s , ttl=%d", hk.String(), tt)
	}
